...
7. The EXE files will be in 'release/bin'
8. Put EXE files, input and output in the same folder and run the model.

# Run the model
The executables should be run at the folder where ‘input’ and ‘output’ folders sit. The following sections describe the input and output files. There is a Python package for creating the input files and visualising the output files. 
...
The running time of HiPIMS mainly depends on four factors:

- The area of the model domain: the increase of domain area can
    linearly escalate the model running time.

- The spatial resolution of the model domain: the increase of spatial
    resolution can exponentially escalate the model running time (twice
    higher resolution approximately means eight times longer running
    time).

- The length of the flood event to be simulated: the increase of event
    duration can linearly escalate the model running time.

- The GPU devices: the increase of the total computing capacity of GPU
    devices can decrease the model running time in near-linear
    efficiency.

For example, with four NVIDIA Tesla K80 GPUS, HiPIMS can simulate a
36-hour event in 18-hour over a 2500-km^2^ domain of 100 million 5 m × 5
m cells, which means twice faster than real time.

# System requirements

The minimum system requirements are as below:
